...
import { OrderLineUpdateRQ } from "@etsoo/smarterp-crm";
...
export default function EditOrderLine() {
  ...
  // Edit data
  const [data, setData] = React.useState<OrderLineUpdateRQ>({ id });
  ...
  const formik = useFormik<OrderLineUpdateRQ>({
    initialValues: data,
    enableReinitialize: true,
    onSubmit: async (values) => {
      // Request data
      const rq: OrderLineUpdateRQ = structuredClone(values);
      ...
      // Submit
      const result = await app.orderLineApi.update(rq);
  ...
  const reloadData = React.useCallback(async () => {
    const result = await app.orderLineApi.updateRead(id);
    if (result == null) return;
        ...
        order?.isDeletable
          ? () => {
              app.notifier.confirm(
                labels.deleteConfirm.format(labels.orderLine),
                undefined,
                async (ok) => {
                  if (!ok) return;

                  const result = await app.orderLineApi.delete(id);
                  if (result == null) return;
