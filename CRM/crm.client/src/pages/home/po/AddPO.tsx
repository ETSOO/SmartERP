...
function formatName(data: QueryForPurchaseData) {
  return data.assignedId ? `${data.assignedId} - ${data.name}` : data.name;
}

function formatPriceLine(
  data: QueryForPurchaseData,
  currencySymbol?: string,
  price?: number
) {
  price ??= app.order.getPOPrice(data);
  return (
    <Typography variant="body2">
      {currencySymbol}
      {price == null ? "???" : app.formatNumber(price)}
      {price != null && data.costPrice != null && price < data.costPrice ? (
        <Typography
          variant="caption"
...
function AddItem({
  ...
  // States
  const [price, setPrice] = React.useState<number | undefined | null>(
    line?.price ?? app.order.getPOPrice(data)
  );
  ...
  const amount = (price ?? 0) * (qty ?? 0);
  const pamount = promotions.reduce((sum, p) => sum + p.amount, 0);

  function changePrice(value: number | undefined) {
    setPrice(value);
  }
    ...
    getValue: (): LocalUtils.OrderLine | undefined => {
      ...
      if (line == null) {
        return {
          ...
          title,
          description,
          originalPrice: data.costPrice ?? 0,
          price,
          qty,
        ...
        <Grid container spacing={1}>
          ...
          >
            <MoneyInputField
              ...
              required
              search
              onChange={(e) => changePrice(NumberUtils.parse(e.target.value))}
            />
...
function SupplierChooser({ data }: { data: LocalUtils.SupplierQueryData }) {
  // Default culture
  data.culture ??= app.system.getDefaultCulture();

  const supplierId = data.supplierId > 0 ? data.supplierId : undefined;
...
function CartList({
  ...
  currencySymbol?: string;
  lines: [LocalUtils.OrderLine, QueryForPurchaseData][];
              ...
              secondaryAction={
                <VBox sx={{ maxWidth: 140 }}>
                  <HBox sx={{ justifyContent: "flex-end" }}>
                    {formatPriceLine(product, currencySymbol, line.price)}
                  </HBox>
...
type SupplierQuery = Omit<QueryForPurchaseRQ, "queryPaging"> & {
  queryPaging: QueryPagingData;
};

type SupplierData = {
  data?: SupplierReadForPurchaseData;
...
export default function AddPO() {
  ...
  const { supplierId } = useSearchParamsEx({
    supplierId: "number"
  });
  ...
  const queryRef = React.useRef<SupplierData>(undefined);

  const [products, setProducts] = React.useState<QueryForPurchaseData[]>();
  const [poLines, setPOLines] = React.useState<LocalUtils.OrderLine[]>([]);
  ...
  const loadProducts = async (reset: boolean = false) => {
    ...
    const items = await app.productApi.queryForPurchase(queryRef.current.query);
  ...
  const loadSupplier = async (
    ...
    // Load supplier data
    const data = await app.supplierApi.readForPurchase({
      supplierId,
      currency
    });
    ...
    // Local storage
    app.storage.setPersistedData(LocalUtils.PO_SUPPLIER_DATA_KEY, {
      supplierId,
      currency,
      culture
    });

    const promotions = data.supplier?.promotions ?? [];
  ...
  // Choose supplier
  const chooseSupplier = (data?: LocalUtils.SupplierQueryData | number) => {
    if (data == null) {
      data = { supplierId: 0, currency: app.system.getDefaultCurrency() };
    } else if (typeof data === "number") {
      data = { supplierId: data, currency: app.system.getDefaultCurrency() };
    }

    app.showInputDialog({
      title: labels.chooseSupplier,
      message: "",
      callback: async (form) => {
        // Cancelled
        if (form == null) {
        ...
        // Form data
        const { supplierId, currency, culture } = DomUtils.dataAs(
          new FormData(form),
          {
            supplierId: "number",
            currency: "string",
        ...
        if (!supplierId || !currency || !culture) {
          return false;
        }
  ...
  // Total amount
  const { cartLabel, currencySymbol, poPromotions } = React.useMemo(() => {
    // Calculate
    const total = poLines.reduce((sum, line) => sum + line.amount, 0);
    const discount = poLines.reduce((sum, line) => sum + line.discount, 0);
    const qty = poLines.reduce((sum, line) => sum + line.qty, 0);

    // Cache
    let currencySymbol: string | undefined = undefined;
    let cartLabel: string | undefined = undefined;
    let poPromotions: PromotionCodeCalculation[] = [];
    if (queryRef.current != null) {
      app.storage.setPersistedData(LocalUtils.PO_LINES_DATA_KEY, poLines);
      ...
      poPromotions = OrderUtils.calculatePromotions(promotions, total);
      let amount = 0;
      for (const p of promotions) {
        const op = poPromotions.find((o) => o.id === p.id);
        if (op) {
          amount += op.amount;
          p.amount = op.amount;
  ...
  }, [poLines, queryRef.current?.query.currency]);
  ...
  async function showCart() {
    ...
    const maxItems = 100;
    if (poLines.length > maxItems) {
      app.notifier.alert(labels.tooManyItemsToDisplay);
      return;
    }

    const products = await app.productApi.queryForPurchase({
      ...queryRef.current.query,
      ids: poLines.map((line) => line.productId),
      queryPaging: maxItems
    });

    if (products == null) return;

    const lines: [LocalUtils.OrderLine, QueryForPurchaseData][] = [];
    ...
    if (emptyLines.length > 0) {
      // Remove
      setPOLines((prev) => prev.filter((line) => !emptyLines.includes(line)));
      ...
      .showInputDialog({
        ...
        callback: (form) => {
          ...
          app.storage.setPersistedData(
            LocalUtils.PO_PROMOTIONS_DATA_KEY,
            pItems
          );

          // Navigate to PO confirmation page
          navigate("./../confirm");
  ...
  React.useEffect(() => {
    // Already chose a supplier
    if (queryRef.current != null) return;

    // Local storage
    const data = app.storage.getPersistedObject<LocalUtils.SupplierQueryData>(
      LocalUtils.PO_SUPPLIER_DATA_KEY
    );
    if (data == null || !data.supplierId) {
      chooseSupplier(supplierId);
    } else {
      const { supplierId, currency, culture } = data;
      loadSupplier(supplierId, currency, culture).then((success) => {
        if (success) {
          // Lines
          const pLines = app.storage.getPersistedObject<LocalUtils.OrderLine[]>(
            LocalUtils.PO_LINES_DATA_KEY
          );
      ...
      <AppBar position="sticky">
        ...
        >
          {searchBarWidth > 0 ? (
            <React.Fragment>
              <IconButton
                onClick={() => chooseSupplier(queryRef.current?.query)}
                title={queryRef.current?.data?.supplier?.name}
                size="small"
              >
                <Avatar>{queryRef.current?.data?.supplier?.name}</Avatar>
              </IconButton>
              {promotions.length > 0 && (
              ...
              <IconButton
                title={cartLabel}
                onClick={poLines.length ? () => showCart() : undefined}
              >
                <Badge badgeContent={poLines.length} color="warning">
                ...
                title={
                  <VBox sx={{ whiteSpace: "wrap" }}>
                    ...
                    >
                      ...
                      {(p.promotions.length === 0 ||
                        !poLines.some((line) => line.productId === p.id)) && (
                        <IconButton
                          color="warning"
                          onClick={() => addPOItem(p)}
                        >
