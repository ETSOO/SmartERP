...
export default function AllPOLines() {
  // Route
  ...
  // Refs
  const ref =
    React.useRef<ScrollerListForwardRef<POLineQueryAllData>>(undefined);
  ...
  return (
    <ResponsivePage<POLineQueryAllData, typeof template>
      {...DefaultUI.pageProps({
      ...
      loadData={async (data) => {
        return await app.poLineApi.queryAll(data, {
          defaultValue: [],
          showLoading: false
        });
      }}
      columns={[
        {
          ...
          cellRenderer: ({
            data
          }: GridCellRendererProps<POLineQueryAllData, BoxProps>) => {
