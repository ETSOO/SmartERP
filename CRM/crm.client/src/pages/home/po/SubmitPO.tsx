...
export default function SubmitPO() {
  ...
  const [poData, setPOData] = React.useState<POData>();
  ...
  const formik = useFormik<DataType>({
    ...
    onSubmit: async (v) => {
      ...
      if (id) {
        ...
        result = await app.poApi.update(rq);
      } else {
        const lines: POLineRQ[] = poData.linesArray.map(
          ({
            productId,
            qty,
            price,
        ...
        const rq: POCreateRQ = {
          ...
          culture: poData.culture,
          amount: poData.amount,
          promotions: poData.promotions,
          lines
        };
        ...
        result = await app.poApi.create(rq);

        if (result?.ok) {
          // Clear local storage
          LocalUtils.clearPOData();
        }
      }
  ...
  const reloadData = React.useCallback(async () => {
    if (!id) return;
    const result = await app.poApi.updateRead(id);
    if (result == null) return;
  ...
  React.useEffect(() => {
    ...
    // Local storage
    const supplier =
      app.storage.getPersistedObject<LocalUtils.SupplierQueryData>(
    ...
    const poDiscount = pPromotions.reduce((sum, item) => sum + item.amount, 0);

    const discount = poDiscount + lineDiscount;

    // PO line amount = line amount - line promotion amount
    const amount =
      pLines.reduce((sum, line) => sum + line.amount, 0) - poDiscount;

    setPOData({
      supplierId: supplier.supplierId,
      currency: supplier.currency,
      symbol: NumberUtils.getCurrencySymbol(supplier.currency),
      culture: supplier.culture,
      discount,
      lines: pLines.length,
  ...
  return (
    ...
    >
      <Grid size={{ xs: 12, sm: 9 }}>
        <SupplierList
          idValue={poData?.supplierId}
          disabled={poData?.hasPromotion}
      ...
      <Grid size={{ xs: 6, sm: 3 }}>
        <CurrencyList value={poData?.currency} fullWidth disabled />
      </Grid>
      <Grid size={{ xs: 6, sm: 3 }}>
        <NumberInputField
          fullWidth
          label={labels.poLines}
          defaultValue={poData?.lines}
          disabled
        />
      </Grid>
      <Grid size={{ xs: 6, sm: 3 }}>
      ...
      <Grid size={{ xs: 6, sm: 3 }}>
        <MoneyInputField
          ...
          defaultValue={poData?.discount}
          symbol={poData?.symbol}
          disabled
        />
      </Grid>
      <Grid size={{ xs: 6, sm: 3 }}>
