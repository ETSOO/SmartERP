...
namespace CRM.Server.Services
{
    ...
    public class PersonService : MyUserService, IPersonService
    {
        ...
        public PersonService(
            ...
            : base(app, config, userAccessor.UserSafe, "person", logger)
        {
            _db = db;
        ...
        public async Task<PersonViewData?> ReadAsync(long id, CancellationToken cancellationToken = default)
        {
            ...
            var data = await _db.Persons.AsNoTracking()
                ...
                .Select(p => new PersonViewData
                {
                    ...
                    Addresses = p.Addresses.Where(a => a.ParentId == null).Select(a => new AddressItem { Id = a.Id, Kind = a.Kind, Name = a.Name, FormattedAddress = a.FormattedAddress }).Take(3).ToArray(),
                    ReportTo = p.ReportTo,
