...
namespace Platform.Server.Endpoints.Member
{
    ...
    public static class Member
    {
        public static RouteGroupBuilder MapMember(this RouteGroupBuilder builder)
        {
            ...
            g.MapPut("AdjustReportTo", [Roles(Constants.HRRoles)] (IMemberService service, MemberAdjustReportToRQ rq, CancellationToken cancellationToken) => service.AdjustReportToAsync(rq, cancellationToken))
                .WithDescription("Adjust member report to / 调整成员汇报对象").WithTags("Member");

            g.MapDelete("Delete/{id:int}", [Roles(Constants.HRRoles)] (IMemberService service, int id, CancellationToken cancellationToken) => service.DeleteAsync(id, cancellationToken))
            ...
            g.MapPost("Invite", [Roles(Constants.HRRoles)] (IMemberService service, MemberInviteRQ rq, CancellationToken cancellationToken) => service.InviteAsync(rq))
                .WithDescription("Invite member / 邀请成员").WithTags("Member");

            g.MapPost("Query", [Roles(Constants.ManagerRoles)] (IMemberService service, MemberQueryRQ rq, IHttpContextAccessor accessor, CancellationToken cancellationToken) => service.QueryAsync(rq, accessor.GetJsonWriter(), cancellationToken))
                .WithDescription("Query members JSON data / 查询成员JSON数据").WithTags("Member");

            g.MapGet("Read/{id:int}", [Roles(Constants.ManagerRoles)] (IMemberService service, int id, IHttpContextAccessor accessor, CancellationToken cancellationToken) => service.ReadAsync(id, accessor.GetJsonWriter(), cancellationToken))
            ...
            g.MapGet("UpdateRead/{id:int}", [Roles(Constants.HRRoles)] (IMemberService service, int id, IHttpContextAccessor accessor, CancellationToken cancellationToken) => service.UpdateReadAsync(id, accessor.GetJsonWriter(), cancellationToken))
