...
{
    /// <summary>
    /// Organization update API request data
    /// 机构更新接口请求数据
    /// </summary>
    public record OrgUpdateApiRQ : UpdateModel<int>, IModelValidator, IOrgRQ
    {
        /// <summary>
        /// Organization id
        /// 机构编号
        ...
        public IActionResult? Validate()
        {
            ...
            if (AppId != null && AppId.Length is not (>= 1 and <= 64))
            {
                return ApplicationErrors.NoValidData.AsResult(nameof(AppId));
            }

            if (AppSecret != null && AppSecret.Length is not (>= 1 and <= 256))
            {
